...
async def test_send_invite_not_found_user(ac: AsyncClient, users_tokens):
    ...
    response = await ac.post("/invite/?user_id=100&company_id=1", json=payload, headers=headers)
    print(response.text)
...
# В моем коде я получаю квери из инвайтов, принадлежащих пользователю
...
async def test_cancel_invite_success(ac: AsyncClient, users_tokens):
    ...
    response = await ac.delete("/invite/4/", headers=headers)
    assert response.status_code == 200
...
async def test_accept_invite_not_your(ac: AsyncClient, users_tokens):
    ...
    response = await ac.get("/invite/5/accept/", headers=headers)
...
async def test_decli_invite_not_your(ac: AsyncClient, users_tokens):
    ...
    response = await ac.get("/invite/6/decline/", headers=headers)
    assert response.status_code == 400
...
async def test_accept_requests(ac: AsyncClient, users_tokens):
    ...
    response = await ac.get("/invite/7/accept/", headers=headers)
    assert response.status_code == 200
